...
export class SessionsHandlerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    (document.querySelectorAll('main section div.thumbnail').item(1) as HTMLDivElement).addEventListener(
      'click',
      () => {
        this.findSelectButtons();
        this.loadCurrentFilter();
  ...
  private findSelectButtons(): void {
    ...
    this.rightSection
      ...
      .forEach((node: HTMLButtonElement) => node.addEventListener('click', () => this.loadCurrentFilter()));
  }

  private loadCurrentFilter(): void {
    this.http
      .get('/rest/sessions/opponents/filter')
      .pipe(map((filters: OpponentFilter[]) => filters.filter((f: OpponentFilter) => !!f)))

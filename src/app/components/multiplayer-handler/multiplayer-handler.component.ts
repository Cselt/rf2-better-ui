...
export class MultiplayerHandlerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    waitForElement('section.content table')
      .then(() => {
        const listItems: NodeListOf<HTMLLIElement> = document.querySelectorAll(
          'section.content section.card section.filter-options li'
        );
